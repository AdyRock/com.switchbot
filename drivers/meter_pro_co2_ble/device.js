...
class MeterProCO2BLEDevice extends Homey.Device
{
	...
	async onAdded()
	{
		this.log('MeterProCO2BLEDevice has been added');
	}
	...
	{
		this.log('MeterProCO2BLEDevice settings where changed');
	}
	...
	{
		this.log('MeterProCO2BLEDevice was renamed');
	}
		...
		try
		{
			...
			if (dd.id)
			{
				...
				if (data)
				{
					...
					this.homey.app.updateLog(`Parsed MeterProCO2 BLE: MeterProCO2 = ${data.serviceData.co2.c}, humidity = ${data.serviceData.humidity}, battery = ${data.serviceData.battery}`, 2);
					this.deviceNotFound = false;
				}
				else
				{
	...
	async syncBLEEvents(events)
	{
		try
		{
			...
			for (const event of events)
			{
				...
				{
					this.setCapabilityValue('measure_temperature', event.serviceData.MeterProCO2.c).catch(this.error);
					...
					this.setCapabilityValue('measure_co2', event.serviceData.co2).catch(this.error);
