...
class VacuumK20HubDevice extends HubDevice
{
	...
	async onAdded()
	{
		this.log('VacuumK20HubDevice has been added');
	}
	...
	{
		this.log('VacuumK20HubDevice was renamed');
	}
	...
	async onCapabilityPlay(value, opts)
	{
		...
		let action = this.getCapabilityValue('robot_vaccum_clean_mode2');
		if (!action)
		{
			action = 'sweep';
		}
	...
	async getHubDeviceValues()
	{
		try
		{
			...
			{
				this.setAvailable();
				this.homey.app.updateLog(`VacuumK20HubDevice got: ${this.homey.app.varToString(data)}`, 3);
		...
		{
			this.homey.app.updateLog(`VacuumK20HubDevice getHubDeviceValues: ${this.homey.app.varToString(err.message)}`, 0);
	...
	async processWebhookMessage(message)
	{
		...
		{
			this.homey.app.updateLog(`VacuumK20HubDevice processWebhookMessage error ${err.message}`, 0);
		}
	}

}

module.exports = VacuumK20HubDevice;
