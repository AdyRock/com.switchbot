...
class MeterProBLEDevice extends Homey.Device
{
	...
	async getDeviceValues()
	{
		try
		{
			...
			if (dd.id)
			{
				this.homey.app.updateLog('Finding MeterPro BLE device', 3, 'ble');
				...
				if (data)
				{
					...
					this.homey.app.updateLog(`Parsed MeterPro BLE: temperature = ${data.serviceData.temperature.c}, humidity = ${data.serviceData.humidity}, battery = ${data.serviceData.battery}`, 2, 'ble');
					this.deviceNotFound = false;
				}
				else
				{
		...
		{
			this.homey.app.updateLog('Finding MeterPro BLE device --- COMPLETE', 3, 'ble');
		}
	}
	...
	async syncBLEEvents(events)
	{
		...
		{
			this.homey.app.updateLog(`Error in MeterPro syncEvents: ${this.homey.app.varToString(error)}`, 0, 'ble');
		}
	}
