...
class BotBLEDevice extends Homey.Device
{
	...
	async onCapabilityOnOff(value, opts)
	{
		...
		{
			this.homey.app.updateLog(`COMMAND: Setting bot state to:${value}`, 'ble');
		...
		this.homey.app.updateLog('COMMAND: Pressing bot', 'ble');
		let cmd = [];
		if (this.pwArr.length > 0)
		{
			cmd = [0x57, 0x11];
	...
	async getDeviceValues()
	{
		...
		try
		{
			...
			if (dd.id)
			{
				this.homey.app.updateLog(`Finding Bot BLE device ${name}`, 2, 'ble');
				...
				else
				{
					...
					if (data)
					{
						...
						this.homey.app.updateLog(`Parsed Bot BLE (${name}): onoff = ${data.serviceData.state}, battery = ${data.serviceData.battery}`, 3, 'ble');
					}
					else
					{
		...
		{
			this.homey.app.updateLog(`Finding Bot device (${name}) --- COMPLETE`, 3, 'ble');
		}
	}
	...
	async syncBLEEvents(events)
	{
		...
		try
		{
			...
			for (const event of events)
			{
				...
				{
					this.homey.app.updateLog(`Got bot state of: ${event.serviceData.state}`, 'ble');
		...
		{
			this.homey.app.updateLog(`Error in Bot (${name}) syncEvents: ${this.homey.app.varToString(error)}`, 0, 'ble');
		}
	}
