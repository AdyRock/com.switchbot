...
class VacuumS10HubDevice extends HubDevice
{
	...
	async onInit()
	{
		...
		this.registerCapabilityListener('robot_vaccum_fill_humi', this.onCapabilityCommand.bind(this, 'addWaterForHumi'));
		...
		this.registerCapabilityListener('robot_vaccum_wash_mop', this.onCapabilitySelfClean.bind(this, '1'));
		this.registerCapabilityListener('robot_vaccum_dry_mop', this.onCapabilitySelfClean.bind(this, '2'));
	...
	async onAdded()
	{
		this.log('VacuumS10HubDevice has been added');
	}
	...
	{
		this.log('VacuumS10HubDevice was renamed');
	}
	...
	async onCapabilityPlay(value, opts)
	{
		...
		let waterLevel = this.getCapabilityValue('vaccum_water_level');
		if (!waterLevel)
		{
			waterLevel = '1';
		}
	...
	{
		const parameter = { action, param: { fanLevel: parseInt(fanLevel, 10), waterLevel: parseInt(waterLevel, 10), times: parseInt(times, 10) } };
	...
	async onCapabilitySelfClean(command, value, opts)
	{
		return this._operateDevice('selfClean', parseInt(command, 10)).catch(this.error);
	}
...
module.exports = VacuumS10HubDevice;
