...
module.exports = class SwitchBotOAuth2Client extends OAuth2Client
{
	...
	async onBuildRequest({ method, path, json, body, query, headers = {} })
	{
		...
		{
			this.homey.app.updateLog(`OAuth2 ${method} ${path} (API calls: ${apiCount})`, 0);
		}
	...
	async onHandleResult({ result, status, statusText, headers })
	{
		if (this.homey && this.homey.app && this.homey.app.updateLog)
		{
			...
			{
				this.homey.app.updateLog(`OAuth2 response ${status}: x-ratelimit-remaining=${remaining}`, 0);
			}
		}
	...
	async onRefreshToken(...args)
	{
		if (this.homey && this.homey.app && this.homey.app.updateLog)
		{
			this.homey.app.updateLog('OAuth2 refresh_token request started', 0);
		}
		...
		try
		{
			...
			if (this.homey && this.homey.app && this.homey.app.updateLog)
			{
				this.homey.app.updateLog('OAuth2 refresh_token request succeeded', 0);
			}
			return token;
		}
		catch (err)
	...
	async onHandleRefreshTokenResponse({ response })
	{
		if (this.homey && this.homey.app && this.homey.app.updateLog)
		{
			...
			if (remaining !== null && remaining !== undefined)
			{
				this.homey.app.updateLog(`OAuth2 refresh_token response: x-ratelimit-remaining=${remaining}`, 0);
			}
		}
	...
	async onGetTokenByCode({ code })
	{
		if (this.homey && this.homey.app && this.homey.app.updateLog)
		{
			this.homey.app.updateLog('OAuth2 authorization_code token request started', 0);
		}
		...
		try
		{
			...
			if (this.homey && this.homey.app && this.homey.app.updateLog)
			{
				this.homey.app.updateLog('OAuth2 authorization_code token request succeeded', 0);
			}
			return token;
		}
		catch (err)
	...
	async onHandleGetTokenByCodeResponse({ response })
	{
		if (this.homey && this.homey.app && this.homey.app.updateLog)
		{
			...
			if (remaining !== null && remaining !== undefined)
			{
				this.homey.app.updateLog(`OAuth2 authorization_code response: x-ratelimit-remaining=${remaining}`, 0);
			}
		}
	...
	async onGetTokenByCredentials({ username, password })
	{
		if (this.homey && this.homey.app && this.homey.app.updateLog)
		{
			this.homey.app.updateLog('OAuth2 password token request started', 0);
		}
		...
		try
		{
			...
			if (this.homey && this.homey.app && this.homey.app.updateLog)
			{
				this.homey.app.updateLog('OAuth2 password token request succeeded', 0);
			}
			return token;
		}
		catch (err)
	...
	async onHandleGetTokenByCredentialsResponse({ response })
	{
		if (this.homey && this.homey.app && this.homey.app.updateLog)
		{
			...
			if (remaining !== null && remaining !== undefined)
			{
				this.homey.app.updateLog(`OAuth2 password response: x-ratelimit-remaining=${remaining}`, 0);
			}
		}
