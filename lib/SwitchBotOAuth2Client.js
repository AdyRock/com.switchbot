...
module.exports = class SwitchBotOAuth2Client extends OAuth2Client
{
	...
	isTransientNetworkError(err)
	{
		...
		return code === 'EAI_AGAIN' || /\bEAI_AGAIN\b/i.test(message);
	}

	async withTransientNetworkRetry(operation, description, maxAttempts = 3)
	{
		for (let attempt = 1; attempt <= maxAttempts; attempt++)
		{
			...
			catch (err)
			{
				...
				{
					this.homey.app.updateLog(`${description} failed with EAI_AGAIN, retrying in ${retryDelay}ms (attempt ${attempt}/${maxAttempts})`, 1, 'hub');
				}
	...
	async onRefreshToken(...args)
	{
		...
		try
		{
			const token = await this.withTransientNetworkRetry(
				() => super.onRefreshToken(...args),
				'OAuth2 refresh_token request',
			);
			if (this.homey && this.homey.app && this.homey.app.updateLog)
			{
	...
	async onGetTokenByCode({ code })
	{
		...
		{
			const token = await this.withTransientNetworkRetry(
				() => super.onGetTokenByCode({ code }),
				'OAuth2 authorization_code token request',
			);
			if (this.homey && this.homey.app && this.homey.app.updateLog)
			{
	...
	async onGetTokenByCredentials({ username, password })
	{
		...
		{
			const token = await this.withTransientNetworkRetry(
				() => super.onGetTokenByCredentials({ username, password }),
				'OAuth2 password token request',
			);
			if (this.homey && this.homey.app && this.homey.app.updateLog)
			{
	...
	async _executeRequest(req, didRefreshToken = false)
	{
		...
		this.debug = function sanitizedDebug(label, ...args)
		{
		...
		try
		{
			...
			const path = req.path || '';
			return await this.withTransientNetworkRetry(
				() => super._executeRequest(req, didRefreshToken),
				`OAuth2 ${method} ${path}`.trim(),
			);
		}
		finally
		{
