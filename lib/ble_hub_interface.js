...
class BLEHubInterface
{
	...
	createBLEHubServer()
	{
		...
		{
			this.homey.app.updateLog(`server got: ${msg} from ${rinfo.address}:${rinfo.port}`, 2);
		...
		server.on('listening', () =>
		{
			...
			this.homey.app.updateLog(`server listening ${address.address}:${address.port}`, 2);
		});
		...
		server.bind(1234, () =>
		{
			...
			this.homey.setTimeout(() => {
				this.homey.app.updateLog('Sending discovery message to SwitchBot BLE Hubs', 2);
	...
	async sendBLEHubCommand(address, command, bestHub)
	{
		try
		{
			...
			this.homey.app.updateLog(`Request to write: ${commandHex} to ${address}`);
	...
	async PostBLEHubURL(url, body, HubAddress)
	{
		...
		return new Promise((resolve, reject) =>
		{
			...
			try
			{
				...
				{
					this.homey.app.updateLog(`Error ${err.code}: ${err.message}`, 0);
					this.postInProgress = false;
